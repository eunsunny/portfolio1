...
public class PageMaker {
	...
	private int cntPageNum = 10;	//화면 화단에 보이는 페이지 버튼의 수
	private int realEndPage;		//전체 항목수에 따른 실제 끝 페이지 번호 저장 
	...
	public void fieldInit() {
		...
		realEndPage = (int)(Math.ceil(totalCount / (double)cri.getRowsPerPage()));						//realEnd = 전체게시글의수 / 페이지당 항목 수 
		...
		prev = startPage == 1 ? false : true;	// 시작페이지가 1페이지일경우 '이전'버튼 필요 없음
		next = endPage * cri.getRowsPerPage() >= totalCount ? false : true;
	}
	
	/*
	 *  화면에서 페이지 번호 클릭할 때, 페이지번호와 페이지당 항목 수를 이용해서  QueryString을 만들어주는 메소드 
