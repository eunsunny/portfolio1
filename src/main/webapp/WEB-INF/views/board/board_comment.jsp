...
<script type="text/javascript">
    ...
    ** 상품평 페이지별 목록 요청
    */
    function getCommentPaging(pagenum, rowsperpage, bseq) {
       $.ajax({
          type: 'GET',
          url: 'board_comments/list',
          dataType: 'json',
		...
		if (commentList.length > 0) {
			...
			if (pageMaker.prev == true) {
				...
				p_html += "<a href='javascript:getCommentPaging("
	                 +pageMaker.startPage-1+","+pageMaker.cri.rowsPerPage+","+${map.bseq}+")'>[이전]</a></li>"
			...
			for(var i=pageMaker.startPage; i<=pageMaker.endPage; i++) {
				p_html += "<a href='javascript:getCommentPaging("
	                 + i +","+pageMaker.cri.rowsPerPage+","+${map.bseq}+")'>["+i+"]</a></li>"
			...
			if (pageMaker.next == true) {
	            ...
	            p_html += "<a href='javascript:getCommentPaging("
	                 +(pageMaker.endPage+1)+","+pageMaker.cri.rowsPerPage+","+${map.bseq}+")'>[다음]</a></li>"
	...
	*/
	 ...
	 */
 	function save_comment(bseq) {
		...
		if(status == 'insert'){
			url = 'board_comments/save';
			data = $("#commentForm").serialize();
		} else{ //status : modify
			url = 'board_comments/update';
			data = {
					bcseq : commentBcseq,
					status : status,
					content : $('#content').val()
	...
	function updateComment(pThis){
		...
		commentBcseq = param.bcseq; // 수정버튼 클릭 시 키값 초기화
		...
		if(status == 'modify'){
			...
			deleteComment(commentBcseq, status);
		}
	}
	
	function deleteComment(bcseq, status){
		$.ajax({
			type 	: 'POST', 
			url  	: 'board_comments/update',
			data 	: {
...
<!--                         <textarea name="ourtext" style="width:100%;height:100;border:1;overflow:visible;text-overflow:ellipsis;">텍스트</textarea> -->
                        ...
                        <td style="width:10%;">
                        <input type="button" id="save" onclick="save_comment('${map.bseq}')" value="등록">
                        </td>
                    </tr>
                </table>
            </div>
